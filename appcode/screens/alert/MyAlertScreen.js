...
import styles from './MyAlertStyles'
// third party
...
class MyAlertScreen extends Component {
    ...
    componentWillUnmount() {
        this.props.Firebase.database().ref('marketAlerts/'+deviceUuid).off('value')
    }
    ...
    componentDidMount() {
        ...
        if(deviceUuid) {
            this.props.Firebase.database().ref('marketAlerts/'+deviceUuid).on('value', (snapshot) => {
    ...
    removeAlertItem(id, i) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if(deviceUuid) {
                ...
                this.props.Firebase.database().ref('marketAlerts/'+deviceUuid).child(i)
                    .remove()
                    .then((response) => {
                        // remove was success
          ...
          <View style={styles.itemBlock}  key={row.id}>
                ...
                <View style={{paddingLeft: 10}}>
                    ...
                    <Text style={styles.mainText}>
                      Rate {direction} {`${row.rate}`}
                    </Text>
                    <Text style={styles.mainText}>
                      <Icon name="clock-o" />  {`${dateCreated}`} {`${timeCreated}`}
                    </Text>
                </View>
